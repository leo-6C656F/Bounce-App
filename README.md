...
| **Tasks** | Action items are extracted after each transcription and collected in a Tasks tab, grouped by recording. Ticking one off survives a re-transcribe. Optionally mirrored to **Apple Reminders** (two-way completion), **Calendar** (for tasks with a deadline), or a **webhook** — each off until you turn it on, and the webhook never includes the transcript. |
...
| **Calendar-aware naming** | Optionally name a recording after the meeting it happened in, from any calendar on the phone. It only names recordings you haven't titled, never writes to your calendar, and won't borrow a meeting name for a short recording. Off by default. |
